...
+ (instancetype)scheduledTimerWithInterval:(NSTimeInterval)interval target:(id)target action:(SEL)action userInfo:(nullable id)userInfo repeats:(BOOL)repeats {
    GGTimer *timer = [self timerWithInterval:interval target:target action:action userInfo:userInfo repeats:repeats];
    [timer schedule];
...
+ (instancetype)scheduledTimerWithInterval:(NSTimeInterval)interval repeats:(BOOL)repeats block:(void (^)(GGTimer *timer))block {
    GGTimer *timer = [self timerWithInterval:interval repeats:repeats block:block];
    [timer schedule];
    return timer;
}
...
- (instancetype)initWithInterval:(NSTimeInterval)interval repeats:(BOOL)repeats block:(void (^)(GGTimer *timer))block {
...
- (void (^)(GGTimer *timer))block {
    void (^block)(GGTimer *timer) = nil;
    [_lock lock];
